...
class Solution {
        ...
        oddSum = 0,
        start = 0;
        int max = getValue(evenSum, oddSum);
        for (int i = 1; i < arr.length; i++) {
        int idx = i - start;
        // including current element into existing subarray
        int value1 = idx % 2 == 0 ? getValue(evenSum + arr[i], oddSum) : getValue(evenSum, oddSum + arr[i]);
        // starting a new subarray at current element
        int value2 = getValue(arr[i], 0);
        ...
        public static int getValue(int evenSum,int  oddSum) {
        return (int)Math.pow(evenSum - oddSum, 2);
        }
